	...
	public function index()
	{
		if(request()->post()){
            ...
            if($usertype=="0"){
				...
				return $this->success('登陆成功',url('index/index'));//管理员zhuye
            }
            if($usertype=="1"){
	            //校验输入
				...
				session('Account',$ret,'admin');
				session('AccountType',1,'admin');
				return $this->success('登陆成功',url('index/index'));//教师zhuye
            }
            if($usertype=="2"){
	            //校验输入
			...
			if($account&&$account->id&&$accountType){
				if( $accountType==0) {return $this->redirect('index/index');}//管理员主页
				if( $accountType==1) {return $this->redirect('index/index');}//教师主页
