	...
	public function index()
	{
		...
		if(request()->post()){
            ...
            if($usertype=="2"){
				...
				return $this->success('登陆成功',url('answer/index'));//学生zhuye
            }
			...
			if($account&&$account->id&&$accountType){
				if( $accountType==3) {return $this->redirect('admin/index');}//管理员主页
				if( $accountType==1) {return $this->redirect('homework/index');}//教师主页
				if( $accountType==2) {return $this->redirect('answer/index');}//学生主页
			}
